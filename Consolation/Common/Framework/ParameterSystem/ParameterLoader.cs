...
namespace Consolation.Common.Framework.ParameterSystem
{
    public static class ParameterLoader
    {
        ...
        public static T GetParameter<T>() where T : class, IParameter => Parameters.First(x => x.GetType() == typeof(T)) as T;
        ...
        /// /// <exception cref="ArgumentNullException"></exception>
        /// <exception cref="InvalidOperationException"></exception>
        public static IParameter GetParameter(string parameter) => Parameters.First(x => x.Name.Equals(parameter) || (x.Aliases ?? Array.Empty<string>()).Contains(parameter));
        ...
        public static void Initialize(Assembly assembly)
        {
            ...
            foreach (Type type in assembly.GetTypes().Where(x => !x.IsAbstract && x.GetConstructor(new Type[0]) != null && Activator.CreateInstance(x) is IParameter))
